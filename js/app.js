(() => {
    ...
    function updateActiveLink(activeIndex) {
        scrollButtons.forEach((button => {
            ...
            if (targetSection && targetSection.hasAttribute("data-fp-id")) {
                const index = parseInt(targetSection.getAttribute("data-fp-id"), 10);
                if (index === activeIndex) button.classList.add("link-active"); else button.classList.remove("link-active");
            }
        }));
    }
    scrollButtons.forEach((button => {
        button.addEventListener("click", (function(e) {
            ...
            if (targetSection && targetSection.hasAttribute("data-fp-id")) {
                const index = parseInt(targetSection.getAttribute("data-fp-id"), 10);
                modules_flsModules.fullpage.switchingSection(index);
                updateActiveLink(index);
            }
        }));
    }));
    document.addEventListener("fpChange", (function(e) {
        const activeIndex = modules_flsModules.fullpage.currentSection;
        updateActiveLink(activeIndex);
    }));
    const tl_sections = gsap.timeline({
        scrollTrigger: {
            trigger: "#prices",
            start: "top 80%",
    ...
    tl_sections.from(".block-prices__item", {
        opacity: 0,
        x: -50,
        duration: .8,
        ease: "power2.out",
